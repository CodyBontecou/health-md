...
| Full Apple `HealthMd-Tests-iOS` scheme on a dedicated iOS 26.5 simulator (unsigned, `CODE_SIGNING_ALLOWED=NO`) | **2258 tests: 2245 passed, 0 failed, 13 skipped — TEST SUCCEEDED**; `SharedSetupV2TransactionScenariosTests` 6/6 against the frozen fixture after the diagnostic removal |
| Android JVM merged suite | Cited cycle-3 receipt (not rerun this cycle; no Android source changed in this lane): 337 tests, 0 failures/errors/skipped, including `SharedSetupV2TransactionScenariosTest` 6/6 — `/tmp/health-md-fleet-loop/cycle-3/integration-report.md` |
