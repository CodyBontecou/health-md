...
Apple workflow secret names are present in `CodyBontecou/health-md`. Two tag-restricted environments were created: `crates-io` accepts only `healthmd-cli/v*` tags, and `google-play` accepts only `android/v*` tags. The `crates-io` environment contains the initial-publication `CARGO_REGISTRY_TOKEN`, and the repository contains `HOMEBREW_TAP_TOKEN`; neither was exercised by publishing during migration. The `google-play` environment contains the Play service-account JSON and existing upload-keystore material required by `.github/workflows/android-release.yml`; values remain encrypted and were never added to Git. No required release secret name identified by this audit remains missing.
