...
- Android: the original phone-only cutover proved unit/direct-protocol tests, a locally signed `:app:bundleRelease`, JAR verification, and temporary read/write service-account access without uploading a build. That historical check is not current release evidence. The Wear-bearing release path has removed Gradle Play Publisher from both application modules and now requires an annotated, main-reachable exact SHA; the protected tag workflow builds the phone/Wear pair, retains immutable pre-mutation AAB/intent evidence, and uploads atomically to `qa`/`wear:qa`. Separate protected evidence and production environments gate one-edit promotion to `production`/`wear:production`. See `apps/android/PLAY_STORE_SETUP.md` and the authoritative Wear completion audit.
...
Apple workflow secret names are present in `CodyBontecou/health-md`. The cutover originally created tag-restricted `crates-io` and `google-play` environments; the latter is superseded for Wear-bearing releases by separate protected `google-play-qa`, `google-play-production`, `wear-evidence-submission`, and read-only `google-play-announce` trust domains. QA holds the existing upload-keystore material but cannot mutate production; production receives no signing material and is gated on sealed evidence and independent review identities. Values remain encrypted and were never added to Git. Current protected-environment configuration and authorization remain release-time gates, not facts established by the historical cutover audit.
