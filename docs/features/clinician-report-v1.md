...
- Android report capture forwards the report's pinned timezone through the repository/provider boundary. Because Health Connect's period-group request has no `ZoneId`, report mode does not use it for activity, heart, vitals, or body: granular records are grouped from pinned instants, and steps are aggregated over each exact zoned local-day instant range. Non-report range callers also aggregate steps over each exact zoned local day (#148); their other activity, heart, vitals, and body metrics retain period aggregation and the system-default API fallback.
