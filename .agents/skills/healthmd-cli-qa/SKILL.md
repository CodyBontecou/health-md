...
- Default CLI export does not write weekly/monthly/yearly roll-up summary files. Use `--use-iphone-settings` only when intentionally testing saved iPhone roll-up behavior.
...
| Exact iPhone settings | Enable roll-ups, run with `--use-iphone-settings` | roll-up summaries are written according to iPhone settings |
