            ...
            case .writeFiles:
                ...
                guard syncService.sendLargePayload(.macExportRequest(job)) else {
                    failPreparation(
                        ...
                        syncService: syncService,
                        reason: .macDestinationUnavailable,
                        message: syncService.lastError ?? "Could not start sending the Mac export job."
                    )
                    return
                }
            case .rawJSON:
                ...
                guard syncService.sendLargePayload(.iphoneExportRawData(payload)) else {
                    failPreparation(
                        ...
                        syncService: syncService,
                        reason: .macDestinationUnavailable,
                        message: syncService.lastError ?? "Could not send raw iPhone export data to Mac."
                    )
                    return
                }
