...
Protocol v2 is an application-layer extension. It deliberately reuses these deployed v1 transport properties from [v1.md](v1.md):
...
The normative Rust models are `crates/healthmd-protocol/src/v2.rs`. Rust/Kotlin interoperability fixtures pin canonical bytes and cryptographic/transfer vectors.
...
The Android contract at `health-md-android/docs/export-contract/raw-snapshot-v1.md` remains authoritative. The artifact is JSON or NDJSON with schema `healthmd.raw-snapshot`, major 1. The manifest also carries provider ID, logical checksum, and snapshot status.
