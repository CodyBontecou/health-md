...
The CLI listens on the computer; the phone connects to the displayed address. It can keep an unavailable request waiting while the user opens Health.md, but this wait-only release cannot launch a suspended app or send APNs/FCM. It never bypasses app activity, permissions, protected-data controls, quotas, or OS background limits. Direct is the portable default. Do not add `--backend mac-app` or `--transport nearby`.
...
MCP uses `HEALTHMD_WAKE_TIMEOUT` (`0` disables) and may emit `notifications/progress`. This P1
release reports notification enrollment as unavailable and does not send a push, so never tell the
user to expect a lock-screen notification. A local timeout or MCP cancellation ends only the
waiter; it is not phone-side durable-job cancellation.
