---
title: "예약된 내보내기"
description: "매일, 매주 또는 사용자 지정 달력 주기로 내보내기를 자동 실행합니다. 보호된 데이터를 사용할 수 없을 때 iOS는 백그라운드 작업과 로컬 복구 알림을 사용합니다."
---
...
<div class="option"><strong>빈도</strong><p>매일, 매주 또는 사용자 지정입니다. 사용자 지정 일정은 시작일을 기준으로 N일, N주 또는 N개월마다 반복됩니다. 되돌아보기 범위는 각 실행에 포함할 완료된 날짜 수를 정합니다.</p></div>
<div class="option"><strong>시간</strong><p>시와 분을 설정합니다. iOS는 이를 보장된 실행 시각이 아니라 목표 시간으로 취급합니다. 아래의 제한 사항을 확인하세요.</p></div>
</div>

## 내보내기 기록
<p>예약 화면 하단의 목록에는 예약된 모든 실행과 그 결과가 기록됩니다. 행을 탭하면 세부 정보를 볼 수 있습니다. 실패한 실행의 <em>재시도</em> 버튼은 현재 구성된 설정과 대상으로 해당 날짜 범위를 다시 실행한 뒤 새 기록 행을 남깁니다.</p>
...
- 각 프로필은 사용자 지정 주기를 포함한 고유 일정을 유지합니다. 활성 프로필을 바꿔도 다른 일정의 대상은 바뀌지 않습니다.
- 같은 대상에 같은 생성 경로를 쓸 가능성이 있는 프로필에는 충돌 경고가 표시됩니다. 경쟁하는 일정을 활성화하기 전에 확인하세요. Health.md는 프로필을 자동으로 바꾸지 않습니다.
- 중지 또는 취소는 현재 시도만 끝냅니다. 완료 날짜는 유지되고, 미해결 날짜는 재시도할 수 있으며, 일정은 계속 활성화됩니다.
- 각 기록 행은 실행 당시 프로필과 실제 대상의 개인정보 보호 레이블에 고정됩니다.

고정 설정과 대상은 [내보내기 프로필](/ko/docs/export-profiles/)에서 관리하세요.

## 관련 문서

<div class="related">
  <a href="/ko/docs/export-profiles/"><span>프로필</span>독립적인 일정과 대상을 관리합니다.</a>
