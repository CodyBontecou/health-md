...
## 선택한 폴더를 안전하게 교체 또는 이동

저장된 북마크가 다른 경로로 확인되면 영구 ID가 같은 폴더임을 증명하는 경우 Health.md가 자동으로 다시 연결합니다. 저장된 폴더와 확인된 폴더 모두 영구 ID를 제공하지 않아도 보안 범위 북마크가 정상적으로 확인되면 수락할 수 있습니다. 이는 클라우드 제공자에서 흔합니다. 비슷한 경로만으로 같은 폴더라고 판단하지는 않습니다. 기록에는 각 실행에서 실제로 사용한 개인정보 보호 대상 레이블이 그대로 남습니다.

폴더가 삭제되었거나 접근 권한이 취소되었거나 영구 ID가 충돌하거나 한쪽에만 ID가 있어 이동을 확인할 수 없다면 폴더를 다시 선택하세요. Health.md는 모호한 대상에 쓰지 않습니다. 각 [내보내기 프로필](/ko/docs/export-profiles/)이 자체 대상을 가지므로 영향을 받는 프로필마다 폴더를 확인하거나 다시 선택하세요.

## 관련 문서

<div class="related">
  <a href="/ko/docs/export-profiles/"><span>프로필</span>프로필별 폴더 접근과 대상을 관리합니다.</a>
