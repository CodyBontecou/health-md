...
<div><strong>v8</strong><span>Apple公開エクスポートスキーマ</span></div>
...
<div class="related">
  ...
  <a href="/ja/docs/cli-direct/"><span>プレビュー · ポータブルCLI</span>スマートフォンへの直接アクセス — Manual IPまたはTailscaleのペアリングと、現在の未認定iPhone／Android互換性マトリクスを確認します。</a>
  <a href="/ja/docs/cli-extract/"><span>ソースデータ</span>正規抽出 — 選択したschema-v8の日付、ソースレコード、プロジェクション、またはJSONLを取得します。</a>
...
<div class="related">
  ...
  <a href="/ja/docs/reference/daily-records/"><span>スキーマv8</span>日次レコード — 公開ソースドキュメントとその所有権ルールを理解します。</a>
...
### 信頼できるプロファイルベースのエクスポート

- 概要または共通の詳細時系列を選びます。正規のロスレスアーカイブはApple限定です。
- 独立した設定、保存先、スケジュールをiPhoneまたはAndroidの端末内プロファイルに保存します。
- 停止またはキャンセルは現在の試行だけに作用し、完了日は維持、未解決日は再試行可能、スケジュールは有効なままです。

安定ID、自動化、保存先履歴、安全な失敗については[エクスポートプロファイル](/ja/docs/export-profiles/)から始めてください。

<div class="related">
  <a href="/ja/docs/export-profiles/"><span>再利用可能なワークフロー</span>エクスポートプロファイル — 保存先、形式、指標、スケジュール、自動化IDを固定します。</a>
  ...
  <a href="/ja/docs/scheduling/"><span>バックグラウンド</span>スケジュール — 繰り返し頻度、復旧、プラットフォームごとの時刻制約を理解します。</a>
  <a href="/ja/docs/shortcuts/"><span>自動化</span>ショートカットとApp Intents — Appleのワークフローからエクスポート、要約、ステータス確認を実行します。</a>
</div>

<p style="margin-top:48px; color:var(--sl-color-gray-3); font-size:12px; font-family:var(--sl-font-mono);">ドキュメント構成の更新日：2026-08-31</p>
