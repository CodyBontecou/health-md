...
- ツールが新しいデータの読み取りまたはエクスポートを開始するときは、ペアリング済みのiPhoneでHealth.mdを開いていること
...
Codexを再起動し、`healthmd_doctor`を呼び出してから、`healthmd_metrics`で指標を一覧表示し、小規模な`healthmd_metric_chart`を要求します。対話型MCP Appsに対応していないホストでも、正確なJSONと標準PNGチャートを受け取れます。
...
同梱Macサーバーは、仕様が固定された21個のツールを公開します。ポータブル版プレビューは、同じ準備状況、分析、生成ファイルエクスポートのツールを公開しますが、暗号化コンテキスト取得用の4ツールは含みません。
...
ネットワークリスナーを開いたりiPhoneへ接続したりせず、同じスキーマをローカルで確認できます。
...
最初に`healthmd_doctor`を呼び出します。`healthmd_metrics`でメトリックIDを解決してから、直接スコープを指定した時系列のチャートを作成します。各クエリは、上限付きの新規読み取りをiPhoneへ明示的に要求します。
...
型付きクエリツールが接続するのは、ペアリング済みで前面表示中のiPhoneだけです。iPhoneは要求日を取得し、コンパクトな型付きコンテキストへプロジェクションして、ローカルでリクエストを評価します。その後、カバレッジ、欠損、エビデンス、制限事項を含む上限付きレスポンスページを返します。

## 生成ファイルをエクスポートする

最初に、コンピューター上に既存の保存先ディレクトリを作成します。ホストが引数全体を表示し、ユーザーが承認した後で、`healthmd_export_files`を呼び出します。
...
ヘルパーには、prompts、roots、sampling、shell、SQL、任意のファイル読み取り、任意のURL取得、HealthKitへの書き込み、ループバックHTTPサービス、リモートMCPエンドポイントがありません。唯一のMCPリソースは、同梱のAppドキュメントです。生成ファイルへの書き込みは、承認が必要な1つの固定操作です。明示した既存の保存先を転送前に検証し、永続的に紐づける必要があります。
...
| エクスポート先を利用できない | 既存の絶対パスで、シンボリックリンクではないデスクトップディレクトリを作成して渡します。 |
