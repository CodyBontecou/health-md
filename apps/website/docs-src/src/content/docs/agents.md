...
4. For Claude or manual setup, configure the absolute `healthmd` path with arguments `mcp serve`. Follow [Health.md MCP server and App](/docs/mcp/).
...
For portable MCP clients, call `healthmd_doctor`. It checks local direct trust and the connected foreground iPhone without reading health values. It returns actionable health-free errors. Each typed MCP query sends a fresh request to that iPhone. The iPhone captures only the requested scope, evaluates the typed query on-device, and returns bounded pages.
...
Portable MCP uses the paired direct protocol. It provides native credential storage, mutual transcript authentication, encrypted packets, and replay protection. A foreground iPhone connects to the computer's explicit address. The optional Mac query API listens only on IPv4 and IPv6 loopback. It validates the peer as loopback.
...
Query values are tagged. A result can contain a quantity and canonical unit, duration, signed count, string, category, or Boolean. It can also contain a UTC timestamp, calendar date, nested array, or unknown future typed payload.
