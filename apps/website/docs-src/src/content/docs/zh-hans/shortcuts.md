...
description: "七个 App Intent 可让您通过 Siri、快捷指令 App、专注模式过滤器、自动化及其他支持 App Intent 的宿主触发导出、获取摘要和切换计划。"
---
...
<a id="profiles"></a>
## 导出配置文件
<p>可在 Health.md 的 iPhone 或 Android 版本中创建和管理已保存的导出配置文件。在 Apple 平台上，配置文件管理目前仅针对 iPhone 提供文档；不宣称在 iPad 或 macOS 上可用。</p>
<p>全部四个导出意图都接受可选的<em>配置文件</em>参数。存在配置文件时，留空会使用活动配置文件；在没有配置文件的旧版模式下，则使用应用当前的导出设置。传入已保存配置文件的名称后，无论应用当前显示什么，都会使用该配置文件冻结的配置（指标选择、格式和目标）运行。</p>
<div class="callout">
