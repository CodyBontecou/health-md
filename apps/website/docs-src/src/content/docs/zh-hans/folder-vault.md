...
保存的书签解析到不同路径时，如果持久身份能够证明它仍是同一文件夹，Health.md 会自动重新绑定。如果保存的文件夹和解析后的文件夹都不提供持久身份，但安全范围书签能成功解析，应用也可以接受；这种情况在云提供商中很常见。仅凭相近路径绝不会认定为同一文件夹。历史仍会显示每次运行实际使用的隐私安全目标标签。

如果文件夹已删除、访问权限被撤销、持久身份冲突，或只有一侧提供身份而无法验证移动，请重新选择文件夹。Health.md 不会写入含糊不清的目标。由于每个[导出配置文件](/zh-hans/docs/export-profiles/)都有自己的目标，请为每个受影响的配置文件核对或重新选择文件夹。

## 相关内容

<div class="related">
  <a href="/zh-hans/docs/export-profiles/"><span>配置文件</span>按配置文件管理文件夹访问和目标。</a>
