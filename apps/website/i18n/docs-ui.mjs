...
export const docsSidebar = Object.freeze([
  Object.freeze({
    ...
    items: Object.freeze([
      ...
      item(text('Connect a local agent', 'Conecta un agente local', 'Lokalen Agenten verbinden', 'Connecter un agent local', 'Conecte um agente local', 'Collega un agente locale', 'Lokale agent koppelen', 'ローカルエージェントを接続', '로컬 에이전트 연결', '连接本地智能体'), 'configuration'),
