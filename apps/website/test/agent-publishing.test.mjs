...
test('published agent assets are byte-exact and checksum-backed', async () => {
  ...
  const publishedSkill = await read('docs-src/public/agents/skills/healthmd-cli/v6/SKILL.md');
  ...
  assert.equal(skillManifest.latest.version, 6);
  assert.equal(skillManifest.latest.path, '/agents/skills/healthmd-cli/v6/SKILL.md');
  ...
  assert.equal(skillManifest.versions.length, 6);
  ...
  assert.deepEqual(skillManifest.versions[5], skillManifest.latest);
