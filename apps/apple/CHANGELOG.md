...
### Fixed
- Persisted export-history file counts now share one cross-category budget instead of being clamped per category, so absurd per-category counts can no longer inflate the persisted generated-file total beyond the supported bound. Breakdowns whose counts were reduced by the budget are marked with an explicit truncated flag so readers can tell exact totals from budget-limited ones, while history saved by earlier versions keeps decoding unchanged.
