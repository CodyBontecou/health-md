...
enum ExportRolloutCopy {
    ...
    static let rollupSummariesHelp = String(localized: "Range summaries are off by default. Current exports write one `\(HealthRollupExportSchema.identifier)` v9 aggregate for the immutable requested range; these summaries are not daily records. Historical weekly, monthly, and yearly v8 artifacts remain valid and documented. Range Summary is available only for ranges of 10,000 days or fewer; existing daily export behavior and limits are unchanged. Turn on Range summary only to skip daily files and write just the range summary.")
