...
enum ExportPathPlanner {
    ...
    nonisolated static func normalizedPortableRelativePath(_ relativePath: String) throws -> String {
        ...
        let normalizedComponents = rawComponents
            .filter { !$0.isEmpty && $0 != "." }
            .map(String.init)
        let windowsInvalidCharacters = CharacterSet(charactersIn: "<>:\"|?*")
        let windowsReservedNames: Set<String> = [
            "CON", "PRN", "AUX", "NUL",
            "COM1", "COM2", "COM3", "COM4", "COM5", "COM6", "COM7", "COM8", "COM9",
            "LPT1", "LPT2", "LPT3", "LPT4", "LPT5", "LPT6", "LPT7", "LPT8", "LPT9"
        ]
        guard !normalizedComponents.isEmpty,
              normalizedComponents.allSatisfy({ component in
                  ...
                  return !component.isEmpty
                      ...
                      && component.last != "."
                      && component.last != " "
                      && !component.unicodeScalars.contains(where: windowsInvalidCharacters.contains)
    ...
    nonisolated static func validatePortableArtifactPaths(_ relativePaths: [String]) throws {
        var keyedPaths: [(path: String, key: String)] = []
        var keys = Set<String>()
        keyedPaths.reserveCapacity(relativePaths.count)
        for relativePath in relativePaths {
            _ = try validatedPortableRelativePath(relativePath)
            let key = try canonicalPortablePathKey(relativePath)
            guard keys.insert(key).inserted else {
                throw PathValidationError.invalidRelativePath(relativePath)
            }
            keyedPaths.append((relativePath, key))
        }

        for keyedPath in keyedPaths {
            let components = keyedPath.key.split(separator: "/", omittingEmptySubsequences: false)
            guard components.count > 1 else { continue }
            var parent = ""
            for component in components.dropLast() {
                parent = parent.isEmpty ? String(component) : parent + "/" + component
                if keys.contains(parent) {
                    throw PathValidationError.invalidRelativePath(keyedPath.path)
                }
            }
        }
    }
    ...
    static func validateUniqueDestinationArtifactPaths(
        ...
        var keyedResolvedPaths: [(relativePath: String, path: String)] = []
        for relativePath in artifactRelativePaths {
            ...
            keyedResolvedPaths.append((relativePath, target.path))
        }
        for target in keyedResolvedPaths {
            var parent = URL(fileURLWithPath: target.path, isDirectory: false)
                .deletingLastPathComponent()
                .standardizedFileURL.path
            while parent != "/" && !parent.isEmpty {
                if resolvedPaths.contains(parent) {
                    throw PathValidationError.invalidRelativePath(target.relativePath)
                }
