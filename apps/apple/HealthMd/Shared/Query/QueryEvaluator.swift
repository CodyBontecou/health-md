...
nonisolated struct HealthMdQueryEvaluator: Sendable {
  ...
  private let cursorBinding: String
  private let datasetFingerprint: String

  init(days: [HealthMdCompactContextDay], cursorKey: Data, cursorBinding: String = "local") throws {
  ...
  func evaluate(
    ...
    guard request.page.maxItems > 0,
      ...
      request.page.maxBytes <= HealthMdPageControls.maximumBytes
    else {
    ...
    case .periodComparison(let first, let second, _):
      try validate(first, within: request.dates)
      try validate(second, within: request.dates)
    default:
      break
    }
    ...
    case .metricSeries:
      ...
      let page = try paginate(
        candidates, offset: offset, controls: request.page, fingerprint: fingerprint)
    ...
    case .periodComparison(let first, let second, let descriptors):
      ...
      let page = try paginate(
        items, offset: offset, controls: request.page, fingerprint: fingerprint)
      let comparisonDays = try selectDays(.exact(combined(first, second)))
      let comparedMetricIDs = Set(descriptors.map(\.metricID))
      let valueDays = Set(
        comparisonDays.filter { day in
          day.metrics.contains {
            comparedMetricIDs.contains($0.metricID) && $0.status == .available && $0.value != nil
          }
        }.map(\.ownerDate))
      return HealthMdQueryResponse(
        items: page.values,
        packet: nil,
        coverage: coverage(
          for: comparisonDays, requested: .exact(combined(first, second)), valueDays: valueDays),
    ...
    case .workoutListing:
      ...
      let page = try paginate(
        workouts, offset: offset, controls: request.page, fingerprint: fingerprint)
      let listedWorkoutIDs = Set(
        workouts.compactMap { item -> String? in
          guard case .workout(let workout) = item else { return nil }
          return workout.workoutID
        })
      let valueDays = Set(
        selectedDays.filter { day in
    ...
    case .sleepSessionListing(let window, let includeNaps):
      ...
      return HealthMdQueryResponse(
        ...
        coverage: coverage(
          ...
          valueDays: Set(
            listing.items.compactMap { item in
    ...
    case .derivePacket(let kind, let detailIDs):
      guard let scope = evidenceScope else {
        throw HealthMdQueryContractError.scopeViolation("missing_evidence_scope")
      }
      let facts = try packetFacts(
        kind: kind, detailIDs: detailIDs, days: selectedDays, selection: request.metrics,
        scope: scope)
      let page = try paginate(
        facts, offset: offset, controls: request.page, fingerprint: fingerprint)
  ...
  private func evidenceIsAuthorized(
    ...
    if let allowed = scope.allowedSourceIDs, !allowed.contains(evidence.reference.sourceID) {
      return false
    }
  ...
  private func evidencePassesSourceRestriction(
    ...
    let scopeRestrictsSources =
      scope.allowedSourceIDs != nil
  ...
  private func validate(
    _ range: HealthMdDateRange,
    within selection: HealthMdDateSelection
  ) throws {
    try validate(range)
    guard case .exact(let selected) = selection else { return }
    guard range.startDate >= selected.startDate,
      range.endDate <= selected.endDate
    else {
      throw HealthMdQueryContractError.invalidDateRange
    }
  }
  ...
  private func selectedMetricIDs(
    _ selection: HealthMdMetricSelection, in selectedDays: [HealthMdCompactContextDay]
  ) -> Set<String> {
    switch selection {
  ...
  private func metricItems(days: [HealthMdCompactContextDay], selection: HealthMdMetricSelection)
    -> [HealthMdQueryItem]
  {
  ...
  private func sourceRecordItems(
    ...
    return days.flatMap { day -> [HealthMdQueryItem] in
      ...
      return day.evidence.compactMap { evidence in
        ...
        let associated = Set(evidence.metricIDs).union(
          linkedMetrics[evidence.reference.evidenceID] ?? [])
        switch metrics {
        case .explicit:
          guard !associated.isDisjoint(with: requestedMetrics) else { return nil }
        case .allAvailable:
          guard associated.isEmpty || !associated.isDisjoint(with: requestedMetrics) else {
            return nil
          }
        }
  ...
  private func sleepSessionItems(
    ...
    for day in selectedDays {
      for session in day.sleepSessions {
        ...
        guard
          evidencePassesSourceRestriction(
        ...
        guard
          let result = HealthMdSleepSessionQuery.result(
            session: session,
            ownerDay: day,
            relatedDays: related,
            window: window,
    ...
    items.sort { lhs, rhs in
      guard case .sleepSession(let first) = lhs,
        case .sleepSession(let second) = rhs
      else { return false }
  ...
  private func adjacentDays(
    ...
    for ownerDate in selectedOwnerDates {
      guard let center = formatter.date(from: ownerDate),
        formatter.string(from: center) == ownerDate
      else { continue }
  ...
  private func sessionCalendarDates(
    ...
    let timeZone =
      TimeZone(identifier: ownerDay.calendarTimeZone)
  ...
  private func workoutSleepAlignmentItems(
    ...
    let normalizedActivity = workoutActivity?.trimmingCharacters(in: .whitespacesAndNewlines)
      .lowercased()
    let sleepCandidates = relatedDays.flatMap { day in
      day.sleepSessions.compactMap {
        session
    ...
    let workoutValues = workoutItems(in: selectedDays).compactMap {
      item -> HealthMdContextWorkout? in
      guard case .workout(let workout) = item else { return nil }
      return workout
    }
    ...
    for workout in workoutValues {
      guard
        normalizedActivity == nil
          || workout.activity.lowercased() == normalizedActivity
      else {
        activityExcluded += 1
        continue
      }
    ...
    items.sort { lhs, rhs in
      guard case .workoutSleepAlignment(let first) = lhs,
        case .workoutSleepAlignment(let second) = rhs
      else { return false }
  ...
  private func comparisonItems(
    ...
    guard !descriptors.isEmpty else {
      throw HealthMdQueryContractError.invalidAggregation("empty_descriptors")
    }
    let descriptorMetricIDs = descriptors.map(\.metricID)
    guard Set(descriptorMetricIDs).count == descriptorMetricIDs.count else {
      throw HealthMdQueryContractError.invalidAggregation("duplicate_metric_id")
    }
    let unselected = Set(descriptorMetricIDs).subtracting(selected)
    guard unselected.isEmpty else {
      throw HealthMdQueryContractError.invalidAggregation("metric_not_selected")
    }
    return try descriptors.sorted { $0.metricID < $1.metricID }.map { descriptor in
      ...
      let exactCounts: (first: Int64, second: Int64)?
      if case .count(let firstCount)? = firstValue,
        case .count(let secondCount)? = secondValue
      {
        exactCounts = (firstCount, secondCount)
      } else {
        exactCounts = nil
      }
      ...
      if let exactCounts {
        direction =
          exactCounts.second == exactCounts.first
          ? .unchanged
          : (exactCounts.second > exactCounts.first ? .increased : .decreased)
      } else if let lhs = firstNumber, let rhs = secondNumber {
      ...
      var percent: Double?
      if let exactCounts {
        if exactCounts.first == 0 {
          limitations.append(
            .init(
              code: "zero_baseline",
              message: "Percent change is unavailable because the first period value is zero."))
        } else {
          let (delta, overflow) = exactCounts.second.subtractingReportingOverflow(exactCounts.first)
          guard !overflow else { throw HealthMdQueryContractError.nonFiniteNumber }
          percent = (Double(delta) / abs(Double(exactCounts.first))) * 100
        }
      } else if let lhs = firstNumber, let rhs = secondNumber {
        if lhs == 0 {
          limitations.append(
      ...
      return .comparison(
        HealthMdPeriodComparison(
          ...
          coverage: coverage(
            for: selectedDays, requested: .exact(combined(first, second)), valueDays: valueDays),
  ...
  private func metrics(for metricID: String, range: HealthMdDateRange) -> [HealthMdMetricPoint] {
    metricItems(
      days: days.filter { $0.ownerDate >= range.startDate && $0.ownerDate <= range.endDate },
      selection: .explicit([metricID])
    ).compactMap {
      if case .metric(let metric) = $0, metric.value != nil, metric.status == .available {
        return metric
      }
      return nil
    }
  ...
  private func aggregate(
    _ metrics: [HealthMdMetricPoint], descriptor: HealthMdAggregationDescriptor
    ...
    let counts = values.compactMap { value -> Int64? in
      guard case .count(let count) = value else { return nil }
      return count
    }
    if counts.count == values.count {
      guard descriptor.expectedUnit == nil || descriptor.expectedUnit == "count" else {
        throw HealthMdQueryContractError.invalidAggregation("unit_mismatch:\(descriptor.metricID)")
      }
      let total = counts.reduce(Decimal.zero) { partial, value in
        partial + Decimal(value)
      }
      switch descriptor.kind {
      case .sum, .durationSum:
        guard let total = Int64(NSDecimalNumber(decimal: total).stringValue) else {
          throw HealthMdQueryContractError.invalidAggregation(
            "count_overflow:\(descriptor.metricID)")
        }
        return .count(total)
      case .average:
        let average = total / Decimal(counts.count)
        if let exact = Int64(NSDecimalNumber(decimal: average).stringValue) {
          return .count(exact)
        }
        let approximate = NSDecimalNumber(decimal: average).doubleValue
        guard approximate.isFinite else {
          throw HealthMdQueryContractError.nonFiniteNumber
        }
        return .quantity(value: approximate, unit: "count")
      case .minimum:
        return counts.min().map(HealthMdQueryValue.count)
      case .maximum:
        return counts.max().map(HealthMdQueryValue.count)
      case .latest, .count:
    ...
    guard pairs.count == values.count, let first = pairs.first else {
      throw HealthMdQueryContractError.invalidAggregation(descriptor.metricID)
    }
    let units = Set(pairs.compactMap { $0.1 })
    guard units.count <= 1, descriptor.expectedUnit == nil || descriptor.expectedUnit == first.1
    else {
      throw HealthMdQueryContractError.invalidAggregation("unit_mismatch:\(descriptor.metricID)")
    }
  ...
  private func typedNumeric(_ value: Double, shape: ValueShape, unit: String?) -> HealthMdQueryValue
  {
    ...
    case .count:
      if let count = Int64(exactly: value) { return .count(count) }
      return .quantity(value: value, unit: unit ?? "count")
    }
  }

  private func difference(first: HealthMdQueryValue?, second: HealthMdQueryValue?) throws
    -> HealthMdQueryValue?
  {
    guard let first, let second else { return nil }
    if case .count(let lhs) = first, case .count(let rhs) = second {
      let (delta, overflow) = rhs.subtractingReportingOverflow(lhs)
      guard !overflow else { throw HealthMdQueryContractError.nonFiniteNumber }
      return .count(delta)
    }
    guard let lhs = first.finiteNumericValue, let rhs = second.finiteNumericValue else {
      return nil
    }
  ...
  private func packetFacts(
    ...
    guard deniedDetails.isEmpty else {
      throw HealthMdQueryContractError.scopeViolation(
        "detail_ids:\(deniedDetails.sorted().joined(separator: ","))")
    }
    ...
    for day in days {
      for metric in day.metrics
      where requested.contains(metric.metricID) && metric.status == .available {
  ...
  private static let medicalSafetyLimitation = HealthMdLimitation(
    ...
    message:
      "This packet reports stored observations only and does not diagnose conditions or recommend treatment."
  )
  ...
  private func metricCoverage(
    ...
    let valueDays = Set(
      points.filter { $0.value != nil && $0.status == .available }.map(\.ownerDate))
    var result = coverage(for: selectedDays, requested: requested, valueDays: valueDays)
    let unavailable = Dictionary(
      grouping: points.filter { $0.value == nil || $0.status != .available }, by: \.ownerDate)
    ...
    if valueDays.isEmpty, Set(missing.map(\.status)).count == 1 {
      overall = missing.first?.status ?? result.status
    } else {
      overall = .partial
    }
  ...
  private func combined(_ first: HealthMdDateRange, _ second: HealthMdDateRange)
    -> HealthMdDateRange
  {
    .init(
      startDate: min(first.startDate, second.startDate), endDate: max(first.endDate, second.endDate)
    )
  }
  ...
  private func normalizedSources(_ sources: [HealthMdSourceDescriptor])
  ...
  private func evidenceIndex(_ days: [HealthMdCompactContextDay]) -> [String:
    HealthMdContextEvidence]
  {
    var result: [String: HealthMdContextEvidence] = [:]
    for item in days.flatMap(\.evidence) where result[item.reference.evidenceID] == nil {
      result[item.reference.evidenceID] = item
    }
    return result
  }
  ...
  private func paginate<Value: Encodable>(
    ...
    guard offset >= 0, offset <= values.count else {
    ...
    let cursor =
      index < values.count ? try makeCursor(offset: index, fingerprint: fingerprint) : nil
...
nonisolated
  ...
  extension HealthMdQueryItem
{
  fileprivate var ownerDateForCoverage: String? {
    switch self {
