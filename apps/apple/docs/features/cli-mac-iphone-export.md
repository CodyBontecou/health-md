...
The compatible default is `--backend mac-app`. The separate, explicit [`--backend direct` iPhone path](./cli-direct-iphone.md) supports pairing, status, canonical `extract`, strict raw, generated-file transfer to `--destination`, resume, and cancel without the SwiftUI Mac app. It never silently falls back to this backend. Query, evidence, refresh, doctor/metrics context, and MCP remain Mac-app-only because they depend on encrypted Mac context or catalog services.
