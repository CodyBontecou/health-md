...
These HTTP routes, encrypted context, and refresh jobs are deliberately available only through the bundled Swift helper's default Mac loopback mode. The helper's `--backend direct` mode returns `backend_unsupported` for those command paths and never falls back; the standalone Rust CLI does not contain them at all. Direct mode supports canonical `extract`, strict raw, and generated-file exports; portable `healthmd mcp serve` adds fresh typed query/evidence, metric catalog, readiness, visualization, and durable export tools over iPhone query protocol v3 without emulating this HTTP API. See [Direct iPhone CLI access](./cli-direct-iphone.md).
