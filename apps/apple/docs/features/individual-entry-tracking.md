...
Individual Entry Tracking creates separate timestamped Markdown files for selected HealthKit source events in addition to the normal daily summary. Typed WHOOP v1 provider records do not participate. With schema v8 and **Lossless Health Records** on, those files derive from canonical HealthKit records rather than inferred daily values.
