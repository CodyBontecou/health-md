...
- **Phase 3 — per-profile scheduled entries — model layer implemented; runtime wiring staged.** `ScheduledExportEntry` (cadence + per-entry progress state, projecting onto the shipped schedule shape so `ScheduleDateMath` applies verbatim), `ScheduledExportEntryStore` (100-entry bound, one entry per profile, legacy-schedule migration into the Default profile, `recordSuccess`), the coalesced evaluator `dueOccurrences(now:)` combining occurrence boundaries with catch-up work per entry, and `ScheduledUsageProjection` for the decision-4 quota surface. Additive plumbing: `PendingExportRequest.profileID/profileName` (decode-compatible; scheduled requests only), `ExportHistoryEntry.profileName`, and `PushRegistrationManager.syncSchedules(_:)` posting one coalesced worker record (earliest enabled preferred time) with fire-time metadata only — the external single-record worker contract is unchanged; a per-entry worker payload needs a worker-side contract change and is deferred.
  Remaining runtime work (device-dependent, the reason it is staged): SchedulingManager's BGTask loop switches from the single schedule to `dueOccurrences` and launches per-entry runs concurrently with per-entry in-flight IDs; notification scheduling labels pending requests with `profileName`; the Schedule tab UI lists per-profile entries with enable/disable, cadence editing, and the projected-monthly-usage surface; ExportHistory recording passes `profileName`; the legacy `ExportSchedule` is retired as the runtime authority once entries own scheduling.
