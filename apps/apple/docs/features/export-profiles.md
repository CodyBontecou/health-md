...
- **Phase 6 — Android parity: model, repository, automation, and scheduling math implemented; scheduler runtime and UI staged.** `ExportProfile` + `ExportProfileRules` (Kotlin mirror of the iOS store contract: trimmed/case-insensitive unique naming, bounded list, one-time Default migration, last-profile deletion guard, fail-closed resolution) with a DataStore-backed `ExportProfileRepository`. `AutomationReceiver` accepts `EXTRA_PROFILE` (id or name) on every export action: empty resolves the active profile once profiles exist, unknown references fail closed with a typed `profile_not_found` protocol error, and profile runs restore the frozen `AndroidExportSettingsSnapshot` onto current settings through the shipped `restoreOnto` path. `ScheduledProfileEntry` + `ScheduledProfileOccurrenceMath` implement the two-layer due semantics (boundary passed **and** trailing-window catch-up pending) for daily/weekly/monthly cadences with per-entry zones, plus `ScheduledProfileUsageProjection` and worker coalescing mirrors. 19 unit tests cover the cross-platform rules contract, occurrence math, usage projection, and coalescing.
  Staged (documented): the per-profile scheduler runtime (AlarmManager arming per entry, WorkManager execution resolving the profile snapshot, recovery) alongside the existing single-schedule `ExportScheduler`; the Android profile-management UI; and the v2 direct-protocol profile resolution mirror. The `export.profiles` capability stays `planned` on both platforms until the Android runtime ships.
