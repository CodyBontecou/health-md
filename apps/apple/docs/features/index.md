...
| Export formats | [JSON export](./json-export.md) | Schema-v8 summaries and typed providers plus the authoritative `healthmd.healthkit_records` v1 source archive. | Drafted | High | `JSONExporter`, `HealthKitRecordArchiveSerializer` |
...
| Export formats | [Export schema contract](./export-schema.md) | Define schema v8, typed provider sections, canonical record completeness, ownership, migration, and parser guidance. | Drafted | High | `HealthMdExportSchema`, `HealthKitRecordArchive`, `ExportSchemaSignatureTests` |
...
| Automation | [Typed queries and evidence packets](./evidence-packets.md) | Bounded, typed metric/workout/coverage queries and factual evidence packets whose values cite their source records. | Drafted | Medium | `HealthMd/Shared/Query/*`, `EncryptedHealthContextQueryExecutor.swift` |
| Automation | [Local MCP server](./local-mcp.md) | Connect Codex or Claude to Health.md through the bundled `healthmd-mcp` helper or portable `healthmd mcp serve` for typed analysis, charts, and approved exports. | Drafted | Medium | `healthmd-mcp` helper, `apps/cli` portable server, `direct-protocol/v3` |
