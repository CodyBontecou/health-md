...
The shared **Sleep Day Attribution** setting (`settings.sleep-attribution`, issue #104) lets the user choose which daily note owns a midnight-spanning sleep session while keeping the whole session in one note: **Night begins** (default) keeps the shipped noon-to-noon night attribution above, and **Morning ends** attributes a session to the calendar date of its end (wake-up date), matching the Health Connect UI. The device-local setting is read live at capture, applies to every capture path, and changes only note ownership — never sleep values, units, reducers, stage identities, or schema shape. Canonical source-record ownership is unaffected either way.
