...
**Data Detail defaults to Summary for new installs.** Detailed Time-Series may add existing optional sample arrays while still reporting `raw_capture_status: not_requested`; that status describes only canonical archive capture. Lossless Health Records adds both readable selected series and the canonical source archive. Historical `includeGranularData` values migrate exactly: `true` becomes Lossless and `false` or missing becomes Summary. New durable settings use independent compatibility-detail and HealthKit-source-archive policies.
