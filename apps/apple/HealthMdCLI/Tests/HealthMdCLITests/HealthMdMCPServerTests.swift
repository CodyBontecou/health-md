...
final class HealthMdMCPServerTests: XCTestCase {
    func testInitializeAndToolListExposeOnlyTools() async throws {
        ...
        let sleep = try XCTUnwrap(
            tools.first { $0["name"] as? String == "healthmd_sleep_sessions" }
        )
        let sleepDescription = try XCTUnwrap(sleep["description"] as? String)
        XCTAssertTrue(sleepDescription.contains("Do not substitute"))
        let sleepSchema = try XCTUnwrap(sleep["inputSchema"] as? [String: Any])
        let sleepExamples = try XCTUnwrap(sleepSchema["examples"] as? [[String: Any]])
        XCTAssertEqual(sleepExamples.first?["all_pages"] as? Bool, true)
        let sleepProperties = try XCTUnwrap(sleepSchema["properties"] as? [String: Any])
        let dates = try XCTUnwrap(sleepProperties["dates"] as? [String: Any])
        XCTAssertEqual((dates["oneOf"] as? [[String: Any]])?.count, 2)
        let includeNaps = try XCTUnwrap(sleepProperties["include_naps"] as? [String: Any])
        XCTAssertEqual(includeNaps["default"] as? Bool, false)
        let page = try XCTUnwrap(sleepProperties["page"] as? [String: Any])
        XCTAssertEqual(page["required"] as? [String], ["max_items", "max_bytes"])
        let query = try XCTUnwrap(tools.first { $0["name"] as? String == "healthmd_query" })
        let querySchema = try XCTUnwrap(query["inputSchema"] as? [String: Any])
        let queryProperties = try XCTUnwrap(querySchema["properties"] as? [String: Any])
        let request = try XCTUnwrap(queryProperties["request"] as? [String: Any])
        let requestProperties = try XCTUnwrap(request["properties"] as? [String: Any])
        let operation = try XCTUnwrap(requestProperties["operation"] as? [String: Any])
        XCTAssertEqual((operation["oneOf"] as? [[String: Any]])?.count, 8)
