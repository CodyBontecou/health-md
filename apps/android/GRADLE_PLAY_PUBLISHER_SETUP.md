...
- `google-play-qa`: existing upload key and a service account restricted to `qa` and `wear:internal`.
...
2. `.github/workflows/android-release.yml` builds/signs both AABs, retains an immutable pre-mutation intent, and uploads phone to `qa` plus Wear to `wear:internal` in one Play edit.
