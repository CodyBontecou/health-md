...
**Date:** 2026-06-20
**Reference:** `docs/export-contract/android-ios-gap-matrix.md` Tier 3, `docs/export-contract/health-connect-phase2-mapping.md`, `docs/export-contract/android-ios-metric-parity-ledger.md`, and `docs/export-contract/platform-native-export-boundaries.md`
...
| Forced Vital Capacity, FEV1, Peak Expiratory Flow, Inhaler Usage | N/A | No standard Health Connect respiratory volume, spirometry, peak-flow, or inhaler-use records in the audited API. |
...
- Medications: HealthKit medication dose events are listed as unavailable because Health Connect has no equivalent medication catalog/dose-event record in the audited API. Android `medical_resources` can export feature-gated PHR/FHIR medication resources, but that is a separate platform-native data model.
