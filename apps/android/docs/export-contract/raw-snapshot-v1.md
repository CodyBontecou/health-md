...
* Interactive, user-triggered exports from the app UI MAY launch Health Connect's per-session consent prompts (bounded per run; the pinned SDK `androidx.health.connect:connect-client:1.2.0-alpha02` has no batched flow) and merge each granted route into the exported record exactly as if Health Connect had returned `ExerciseRouteResult.Data` inline — same `route.state=data`, same location shape, same canonical order.
* A denied, dismissed, or skipped session, and every non-interactive consumer — scheduled exports, automation, and the direct CLI protocol, which has no Activity to host the consent contract — MUST keep exporting `route.state=consent_required` with an empty `locations` array. That state is the machine-readable truth, not an error, and MUST NOT fail or partial the snapshot.
...
All artifact JSON is UTF-8 without a BOM. Canonical JSON recursively sorts object keys by Unicode scalar-value (code-point) order, not UTF-16 code-unit order, emits no insignificant whitespace, and preserves array order. Strings use JSON escaping. Producers MUST reject non-finite numbers. Decimal rendering MUST be locale-independent and round-trippable.
...
`recordCount` and `typeCounts` count survivors. Duplicate elimination is not evidence that the provider returned identical payloads. A native identity group whose canonical hashes differ produces one `identity_collision` issue, increments `identityCollisionCount`, and uses the same winner rule. An identity group whose hashes are identical is deduplicated without that issue. A cross-type identity collision follows the same global rule. Producer external sorts and validator identity checks MUST remain disk-backed and bounded; merge implementations MUST use bounded-fan-in multipass merging rather than opening every spill chunk at once, and MUST delete superseded intermediate chunks.
...
A crash, cancellation, I/O failure, or missing final manifest is **incomplete**, not `PARTIAL`. `PARTIAL` is a deliberate completed artifact with a final manifest and actionable issues. `.partial` files are never consumer artifacts. Sorting, record/issue/report emission, SAF copy, final verification, and the point immediately before promotion MUST observe coroutine cancellation. Storage promotion happens only after bytes are closed. SAF copy/delete is not a cross-provider atomic rename: the exporter MUST recheck final-name absence, reject provider auto-renames, copy in bounded chunks, verify the created final document's exact byte count and SHA-256 against the completed partial, and delete both partial and created final on cancellation or failure.
...
For a folder export, Health.md writes the immutable snapshot under `<configured-subfolder>/raw/` with range, schema version, and snapshot ID in the filename. Promotion returns a receipt for the actual final URI, exact display name, byte count, and verified checksum. Only after that verification it writes `<actual-artifact-name>.sha256` containing the lowercase artifact checksum, two ASCII spaces, the exact promoted artifact filename, and LF. A sidecar MUST be bound to that receipt, never merely to pre-copy partial bytes. A missing sidecar does not invalidate the embedded logical or manifest checksum, but consumers MUST NOT claim exact artifact-byte verification without it.
...
A cloud adapter MUST distinguish exact source payload from parsed projections. It MUST preserve each exact successful page as authoritative base64 plus exact decoded text, content metadata, fetch instant, ordinal, and native checksum when claiming exact fidelity. When strict decoding with the declared charset fails, authoritative bytes remain valid only with `responseText:null`; any non-null contradictory text is invalid. Parsing is allowed only to discover pagination/fan-out values or validate a provider application envelope before capture. It MUST declare pagination and server-side aggregation per endpoint; `serverAggregation=true` never claims record-level data. Exact successful provider bodies can themselves contain response-internal pagination cursors, so the body, text, and base64 remain sensitive. Opaque or secret cursor/token values MUST NOT enter request/query metadata, logs, error text, endpoint identifiers, or exported headers; explicitly allowlisted non-secret numeric pagination offsets may appear as request metadata. OAuth tokens, refresh tokens, request headers, cookies, provider secrets, and provider error bodies MUST never enter records or issues. An adapter with only normalized metrics uses `unsupported_by_provider` for native types it cannot faithfully supply; it MUST NOT synthesize provider-native metadata.
...
| Native cloud payloads | Exact Fitbit, Oura, WHOOP, and Withings pages are streamed through the OAuth client's capture boundary; Withings application errors are rejected before observers. | Never replace them with normalized `HealthData`; retain capped/cycle-detected Oura, WHOOP, and Withings paging and WHOOP cycle-to-recovery provenance. |
...
| Cancellation | External sort, output writing, bounded SAF copy, verification, and promotion gates are cancellation-aware; partial and just-created finals are aborted. | Preserve; a final `CANCELLED` artifact is forbidden. |
...
| Streaming validation/import boundary | `RawSnapshotValidator` validates JSON/NDJSON one record at a time and uses a bounded disk-backed identity index; `RawRecordDecoder` has an exhaustive wire-type switch and typed nested variants. | Keep validation independent from export and reject structural/unit/hash regressions. |
...
| Process restart | Restart-only, from-scratch policy; no Health Connect page token or destination credential is checkpointed. Startup removes abandoned private spools, owned destination `.partial` documents, and stale completed no-backup API artifacts while registry-protecting live concurrent runs and unrelated user files. | Never promote crash debris or place credentials/health data in WorkManager `Data`. |
...
Validation covers canonical record order and disk-backed exact identity uniqueness; original set-array string uniqueness and code-point order before set decoding; record/type/issue counts; scope/status semantics; type-report order and Health Connect's closed inventory and range behavior; raw/manifest/logical checksums; optional exact artifact and sidecar checksums; full-resolution exact integer strings; finite decimal round trips; nullable offset shape; raw+label enums; canonical quantity families and units; nested ordering; provider base64, byte checksum, declared-charset/text agreement, endpoint-identifier syntax; medical wrapper/source/resource consistency; and exact UTF-8 FHIR string checksums. Validation failures contain stable codes and structural locations only. They MUST NOT interpolate record values, provider text, FHIR, route coordinates, credentials, or source issue messages.
...
After process death, the next explicit or scheduled run starts the complete non-transactional snapshot again with a new creation instant. The in-process registry protects live concurrent spool/partial paths. Because that registry is empty in a new process, run startup deletes abandoned installation-private `spool-*` directories, Health.md-owned destination `.partial` documents, and completed no-backup API artifacts before opening the new partial; unrelated user files ending in `.partial` are never cleanup candidates. Final names are created/promoted only after the new manifest is closed. No old partial is promoted, and no page token is resumed. This restart-only behavior is safer and more truthful than token checkpointing across provider mutation.
