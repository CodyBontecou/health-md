...
- Shared Setup wire format: `SharedSetupMapper`/`SharedSetupV2Mapper` write lowercase enum names in `presentation` (`dateFormat`, `timeFormat`, `units`); import maps `imperial` to `IMPERIAL` and anything else to `METRIC`, and decodes date/time values by exact enum name.
