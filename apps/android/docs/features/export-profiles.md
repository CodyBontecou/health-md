...
Export profiles save complete export configurations — metrics, formats, output options, and their own destination — so you can switch between setups with one tap instead of reconfiguring. Each profile carries a stable ID you can pin in automation, its own optional schedule (daily/weekly/monthly cadence, completed-day lookback, and optional Today Refresh that also re-exports the current day every 3, 6, or 12 hours), and an overlap warning when two profiles would write the same files. One profile is always active, and the Export tab edits that active profile directly.
