...
phone=29
wear=1000029
play_assert_distinct_tracks qa wear:qa || fail 'valid form-factor tracks rejected'
if play_assert_distinct_tracks qa production >/dev/null 2>&1; then fail 'non-Wear destination accepted'; fi
if play_assert_distinct_tracks wear:qa wear:production >/dev/null 2>&1; then fail 'Wear phone source accepted'; fi
...
{"track":"wear:qa","releases":[{"name":"wear","status":"completed","versionCodes":["1000029"],"inAppUpdatePriority":5}]}
JSON
