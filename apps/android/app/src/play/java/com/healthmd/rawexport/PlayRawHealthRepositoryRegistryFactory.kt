...
object PlayRawHealthRepositoryRegistryFactory {
    fun create(
        ...
        val cloudProviders: List<CloudNativeRawPageProvider> = listOf(fitbit, withings, oura, whoop)
        val additionalRepositories = buildMap {
        ...
        return RawHealthRepositoryRegistry.withAdditionalRepositories(
...
class UnsupportedRawHealthRepository(
            ...
            if (!selected) {
                ...
                emit(
                    RawExportItem.TypeReport(
                        definition.report(RawTypeStatus.UNSUPPORTED_BY_PROVIDER, message),
                    ),
                )
            }
        }
    ...
    private fun RawProviderTypeDefinition.report(
