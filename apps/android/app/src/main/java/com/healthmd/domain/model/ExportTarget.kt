...
/**
 * Validation and privacy-safe display helpers for the user-configured Agent Data gateway
 * destination. Mirrors the [APIExportEndpoint] discipline exactly: non-empty, no CR/LF, an
 * http/https scheme, a host, and no userinfo/fragment; secrets embedded in query parameters
 * are never displayed or logged.
 */
object AgentDataGatewayEndpoint {
    fun normalizedOrNull(value: String): String? {
        val trimmed = value.trim()
        if (trimmed.isEmpty() || trimmed.contains('\r') || trimmed.contains('\n')) return null
...
object ExportTargetReadiness {
    fun canExport(
        ...
        agentDataGatewayConfigured: Boolean = false,
    ): Boolean {
