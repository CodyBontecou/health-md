...
The simulator rejects rather than guesses relative-date preview materialization, DST gap/overlap materialization, recurring cadence anchors, overnight/touching windows, and other combinations lacking a language-neutral contract oracle. Pulse `required` is represented as a requested policy only and is not interpreted as packet rejection. Renewal is an explicit immutable successor; there is no rollover job.
