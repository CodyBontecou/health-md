...
describe("practice component and production-disabled runtime boundary", () => {
  ...
  it("structurally defines focused mobile policy jobs without reusable full-workflow calls", async () => {
    ...
    expect(workflow).toContain("-project apps/apple/HealthMd.xcodeproj");
    expect(workflow).toContain("-scheme HealthMd");
    ...
    expect(workflow).toContain("SWIFT_ACTIVE_COMPILATION_CONDITIONS");
    expect(workflow).toContain("OTHER_SWIFT_FLAGS");
    expect(workflow).toContain("CODE_SIGNING_ALLOWED=NO");
    expect(workflow).not.toMatch(/\bxcodebuild\b[^\n]*\b(?:build|archive)\b/);
