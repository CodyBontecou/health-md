...
pub fn merge_markdown(existing: &str, generated: &str) -> Result<String, RenderError> {
    if existing.len() > 8 * 1024 * 1024
        || generated.len() > 8 * 1024 * 1024
        || existing.contains('\0')
    ...
    if existing == generated || existing.contains(generated) {
        return Ok(existing.to_owned());
    }
    let (existing_front, existing_body) = split_frontmatter(existing);
    let (generated_front, generated_body) = split_frontmatter(generated);
    let merged_front = merge_frontmatter(existing_front, generated_front);
    let merged_body = merge_sections(existing_body, generated_body);
    let mut output = String::new();
    if !merged_front.is_empty() {
        output.push_str("---\n");
        output.push_str(&merged_front);
        if !merged_front.ends_with('\n') {
            output.push('\n');
        }
        output.push_str("---\n");
    }
...
fn merge_frontmatter(existing: &str, generated: &str) -> String {
    ...
    let mut lines = existing
        .lines()
        .filter(|line| frontmatter_key(line).is_none_or(|key| !generated_keys.contains(key)))
        .map(str::to_owned)
        .collect::<Vec<_>>();
    lines.extend(generated.lines().map(str::to_owned));
    lines.join("\n")
}
...
fn merge_sections(existing: &str, generated: &str) -> String {
    let generated_sections = sections(generated);
    let generated_names = generated_sections
        .iter()
        .map(|section| section.0.as_str())
        .collect::<BTreeSet<_>>();
    let existing_sections = sections(existing);
    let mut output = existing_sections
        .into_iter()
        .filter(|section| !generated_names.contains(section.0.as_str()))
        .map(|section| section.1)
        .collect::<String>();
    if !output.is_empty() && !output.ends_with('\n') {
        output.push('\n');
    }
    for (_, content) in generated_sections {
        output.push_str(&content);
    }
    output
}

fn sections(input: &str) -> Vec<(String, String)> {
    ...
    for line in input.split_inclusive('\n') {
        if let Some(name) = heading_name(line) {
            ...
            line.clone_into(&mut current);
        } else {
            current.push_str(line);
        }
    }
    if !current.is_empty() {
        result.push((current_name, current));
    }
    result
}
...
fn heading_name(line: &str) -> Option<&str> {
    ...
    let value = trimmed[hashes + 1..].trim();
    Some(
        value
