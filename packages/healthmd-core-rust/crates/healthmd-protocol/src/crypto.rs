...
pub fn pairing_verifier(
    ...
    let key = pairing_code_key(pairing_code);
...
pub fn session_key(shared_secret: &[u8; 32], client_nonce: &[u8], server_nonce: &[u8]) -> [u8; 32] {
    // This is the only transcript here that contains key material. Keep it in a
    // zeroizing allocation so the shared-secret copy is wiped on every return path.
    let mut transcript = Zeroizing::new(SESSION_DOMAIN.to_vec());
...
fn pairing_code_key(pairing_code: &str) -> Zeroizing<[u8; 32]> {
...
fn pairing_code_key_with_domain(pairing_code: &str, domain: &[u8]) -> Zeroizing<[u8; 32]> {
    ...
    let mut value = Zeroizing::new(domain.to_vec());
    value.extend_from_slice(normalized.as_bytes());
    Zeroizing::new(Sha256::digest(&*value).into())
}
...
mod tests {
    ...
    #[test]
    fn secret_bearing_intermediates_support_explicit_zeroization() {
        use zeroize::Zeroize as _;

        let mut pairing_key = pairing_code_key("123456");
        pairing_key.zeroize();
        assert_eq!(&pairing_key[..], &[0; 32]);

        let mut session_key = Zeroizing::new(session_key(&[1; 32], &[2; 32], &[3; 32]));
        session_key.zeroize();
        assert_eq!(&session_key[..], &[0; 32]);
    }
